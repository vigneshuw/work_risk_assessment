...
def compute_time_domain_features(x):
    ...
    """
    Compute a set of time domain features
...
def compute_frequency_domain_features(x, args):
    ...
    assert isinstance(args[0], dict), "args should be a list of dictionaries"
...
def compute_energy_fft(x, **kwargs):
    ...
    """
    Energy of FFT - Absolute value of the sum of ffts of the time series signal

    :param x: A time series input
    :return: Energy of FFT
    """
...
def compute_PSD_FFT(x, **kwargs):
    ...
    """
    Welch Power Spectral Density (PSD) for the time series data. The PSD is then multiplied by the difference in the
    first two frequencies

    :param x: The time series input
    :return: Sum of PSD multiplied by difference in the first two frequencies
    """
